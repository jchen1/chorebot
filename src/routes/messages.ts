        ...
        async.waterfall([
          ...
          (nextChores, cb) => {
            const text = _.join(_.map(_.compact(nextChores), obj => {
              ...
              return `${nextUser.username} is now responsible for ${_.lowerCase(chore.name)} (last completed ${moment(chore.lastFinished).toString() || 'never'})`;
            }), '\n');
            async.parallel([
      ...
      case 'remind':
        ...
        async.waterfall([
          ...
          (res, cb) => async.each(res, ({ chore, user }, cb) => {
            if (user) return client.chat.postMessage(config.channel.id, `${user.username} do the ${_.lowerCase(chore.name)}`, cb);
            cb();
          }, cb)
        ], _.noop);
        break;
